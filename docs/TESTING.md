...
| `rosterGenerationService` | Never exceeds the daily labor-hour budget or store monthly capacity; never double-books an employee on the same day; Full-time is always exactly 8 working hours + 1h break (9h clock span), Part-time always 4-8h; guarantees opening (>=1) and closing (>=2) coverage; caps an individual employee at their weekly hours and at 6 consecutive working days; handles a store with zero forecasted sales / no `LaborGuideline` gracefully. |
